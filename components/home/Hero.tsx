...
export default function Hero() {
  return (
    <section className="relative min-h-[85vh] sm:min-h-screen bg-[#0A0F1E] tech-grid bg-grid-pattern bg-noise flex flex-col justify-center overflow-hidden pt-28 pb-20 sm:py-36 border-b border-white/15">
      <TechnicalSignature categoria="DESIGN OPS &amp; BRANDING" />

      {/* MARGINALIA DE TERMINAL (Cyan Elétrico + Logs Monoespaçados Vercel / Linear Style) */}
      <div className="absolute top-6 left-6 font-mono text-[10px] tracking-widest text-blue-neon/70 uppercase select-none pointer-events-none hidden sm:flex items-center gap-2">
        <span className="w-1.5 h-1.5 rounded-full bg-yellow-neon animate-pulse" />
        <span>[ PLX_SYS_CORE: ONLINE ] // LATENCY: 0.04ms</span>
      </div>

      <div className="absolute top-6 right-6 font-mono text-[10px] tracking-widest text-blue-neon/70 uppercase select-none pointer-events-none hidden md:block text-right">
        <span>COORD: 24.5°N 19.8°W // CAD_GRID: 36PX</span>
      </div>
      ...
      {/* MARCADORES DE RETÍCULA CAD (Crosshairs '+') */}
      <div className="absolute top-12 left-12 font-mono text-xs text-blue-neon/30 select-none pointer-events-none hidden lg:block">+</div>
      <div className="absolute top-12 right-12 font-mono text-xs text-blue-neon/30 select-none pointer-events-none hidden lg:block">+</div>
      <div className="absolute bottom-12 left-12 font-mono text-xs text-blue-neon/30 select-none pointer-events-none hidden lg:block">+</div>
      <div className="absolute bottom-12 right-12 font-mono text-xs text-blue-neon/30 select-none pointer-events-none hidden lg:block">+</div>

      {/* ASSET VISUAL: ANIMAÇÃO MAGNÉTICA "DEUS EX MACHINA" + TERMINAL DE TOKENS */}
      <HeroGraphic />

      <div className="shell py-8 sm:py-20 md:py-28 relative z-10 space-y-6">
        {/* Tag Categoria Utilitária em Amarelo Plixel & Cyan */}
        <div className="inline-flex items-center gap-2 bg-[#0A0F1E]/80 backdrop-blur-md border border-blue-neon/40 px-3.5 py-1.5 rounded-none animate-fade-up">
        ...
        {/* H1 Headline B2B (Sans-Serif Geométrica Limpa e de Peso Moderado - Vercel / Bynario Style) */}
        <h1 className="title-impact text-[clamp(32px,5.2vw,74px)] leading-[1.08] text-white max-w-4xl tracking-tight uppercase font-semibold animate-fade-up delay-100 font-sans">
        ...
        {/* BOTÕES UTILITÁRIOS (Amarelo Plixel Destacado + Hover Cortante +4px) */}
        <div className="pt-6 flex flex-wrap gap-4 items-center animate-fade-up delay-300">
          ...
          <Link
            href="/contato"
            className="group bg-[#0A0F1E]/80 backdrop-blur-md border border-white/25 text-white font-mono font-bold text-xs sm:text-sm uppercase tracking-wider rounded-none px-6 py-4 hover:border-yellow-neon hover:text-yellow-neon transition-all duration-200 flex items-center gap-3"
          >
